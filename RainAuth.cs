    ...
    public class api
    {
        ...
        public void Checkinit()
        {
            if (!initzalized)
            {
                if (IsDebugRelease)
                {
                    error("KeyAuth não iniciado.");

                }
                else
                {
                    error("Nova Atualização disponivel");
                }
        ...
        {
            Process.Start(new ProcessStartInfo("cmd.exe", $"/c start cmd /C \"color 08 && title Aviso!!! && echo {message} && timeout /t 5\"")
            {
                CreateNoWindow = true,
            ...
            catch (WebException webex)
            {
                ...
                switch (response.StatusCode)
                {
                    ...
                    default: // site won't resolve. you should use keyauth.uk domain since it's not blocked by any ISPs
                        error("Falha na conexão. Tente novamene mais tarde.");
                        Environment.Exit(0);
                        return "";
                }
            }
